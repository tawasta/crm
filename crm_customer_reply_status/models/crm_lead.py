    ...
    def message_post(self, *args, **kwargs):
        ...
        * False = the salesperson has answered, so we are waiting for the
        ...
        vals = {
            ...
            "customer_reply_date": (
                message.date or fields.Datetime.now()
            ),
        }
        ...
        for lead in self:
            ...
            if lead.team_id.customer_reply_notify:
                ...
                members = (
                    lead.team_id.crm_team_member_ids.filtered(
                        "customer_reply_notify"
                    )
                )

            partners = (
                members.mapped("user_id.partner_id")
                .filtered("active")
            )
            ...
            lead.message_post(
                body=_(
                    "Customer reply email notification sent to: %s"
